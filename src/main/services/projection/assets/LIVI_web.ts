export const buildLiviWeb = (): string =>
  [
    ...
    '    textarea{width:100%;min-height:90px;background:#000;border:1px solid #444;',
    '             border-radius:6px;padding:8px;color:#eee;font-family:ui-monospace,Menlo,Consolas,monospace;',
    '             font-size:12px;line-height:1.35;box-sizing:border-box;}',
    '    pre{background:#0f0f0f;border:1px solid #2a2a2a;border-radius:8px;',
    '        padding:10px;white-space:pre-wrap;overflow-x:auto;max-height:320px;overflow:auto;}',
    '    .muted{color:#aaa;}',
    '    .chip{display:inline-block;background:#1b1b1b;border:1px solid #2a2a2a;',
    ...
    '               padding:10px 0;border-bottom:1px solid #222;gap:12px;}',
    ...
    '            background:#333;border:1px solid #444;transition:.2s;border-radius:999px;}',
    '    .slider:before{position:absolute;content:"";height:20px;width:20px;left:3px;top:2px;',
    '                  background:#bbb;transition:.2s;border-radius:999px;}',
    ...
    '      if (armed) {',
    '        outEl().textContent = "(ARMED) Ready. Running commands executes on the dongle.";',
    '      } else {',
    ...
    '        outEl().textContent = "History:\\n\\n" + prev.map((c,i)=>`${i+1}. ${c}`).join("\\n");',
    '      } catch { outEl().textContent = "Failed to load history."; }',
    '    }',
    '',
    ...
    '      const prev = !el.checked; // state before user action',
    '      lockToggle(el, true);',
    '      try {',
